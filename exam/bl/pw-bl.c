...
char allowIp [4][4] = {{1,1,1,1},{88,80,187,84},{142,250,184,227},{147,162,235,155}};
...
int main()
{
    ...
    while( 1 ){
        ...
        if(!strcmp("GET",method)){
            ...
            char allow = 0;
            for(int y = 0; y< 4; y++) {
                if(!memcmp(he->h_addr, &allowIp[y][0],4)) allow = 1;
            }
            printf("Allow: %d\n",allow);
            if(allow) {
            ...
            char allow = 0;
            for(int y = 0; y< 4; y++) {
                if(!memcmp(he->h_addr, allowIp[y],4)) allow = 1;
            }
            printf("Allow: %d\n",allow);
            if(allow) {
                s3=socket(AF_INET,SOCK_STREAM,0);
